...
def filter_annotations(example):
    ...
    return example["annotation"].startswith('There was a progression') or example["annotation"].startswith('There was a recurrence') or  example["annotation"].startswith('There was no progression/recurrence')
...
def preprocess_function_rag(example):
    few_shot_examples = get_rag_few_shot_examples(example, filtered_train, num_samples=11)
