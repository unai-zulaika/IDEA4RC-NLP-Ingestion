...
#        preds = preds.split('\n')[0]
    ...
    if '[put date]' in preds and '[put date]' in annt:
        preds = preds.replace('[put_date]', '').strip()
        preds = annt.replace('[put_date]', '').strip()       
...
#        print('\nStatus: !!! Unable to Generate !!!')
    if unicodedata.normalize('NFKC', preds) == unicodedata.normalize('NFKC', annt):
        em += 1
        print('\nStatus: ==== Exact Match ====')
    elif (len(preds) == len(annt) and cossim >= 0.9) or cossim == 1.0:
        em += 1
