...
Your task: From the given medical note, identify the tumor location and the exact site.
If the note specifies the tumor location, classify it into the correct category (e.g., Upper and Lower limbs, Trunk wall, Intra abdominal, Genito urinary, Head and neck, Breast).
Then provide the exact site mentioned in the note (e.g., head and neck, Upper arm, Upper leg, Periscapular, Groin, Leg, Forearm, Foot, Buttock, Ankle, Thoracic Wall, Utero, Pelvis, Retroperitoneum, Paravertebral – thoracolumbar, Abdominal wall).

If the tumor site cannot be determined, output "Tumor category/site unknown".

Always present the output in the following structured format and in English:  
Annotation: Tumor site ([Category]): [Specific site].

The output (if present in the note) can only be from one of the following options:
...
def preprocess_function_rag(example):
    few_shot_examples = get_rag_few_shot_examples(example, filtered_train, num_samples=12)
