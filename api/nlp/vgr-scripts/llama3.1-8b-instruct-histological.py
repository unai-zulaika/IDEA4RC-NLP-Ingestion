...
df_filtered = df_filtered.drop_duplicates(
    subset=["note_original_text", "annotation"], keep='first')
...
def get_rag_few_shot_examples(input_example, train_dataset, num_samples):
    ...
    top_indices = sorted(similarities, key=lambda x: x[0], reverse=True)[
        :num_samples]
...
def preprocess_function_rag(example):
    few_shot_examples = get_rag_few_shot_examples(
        example, filtered_train, num_samples=12)
    ...
    model_inputs = tokenizer(
        input_text, max_length=100000, padding="max_length", truncation=True)
    labels = tokenizer(output_text, max_length=100000,
...
prompt_template = generate_prompt_template(
    formatted_few_shot_examples, static_samples)
...
#        print('\n[BEFORE]---Generated (Raw) Annotation:\n', pred1)
        preds = generated_text[0].split('### Response:\n')[1].split('\n')[0].replace(
            'Annotation:', '').strip().split('.')[0].replace('<|im_end|>', '').replace('<br>', '').strip()
#        preds = preds.split('\n')[0]
