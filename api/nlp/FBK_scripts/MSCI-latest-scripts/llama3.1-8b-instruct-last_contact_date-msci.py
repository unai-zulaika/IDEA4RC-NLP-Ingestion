...
def filter_annotations(example):
    ...
    return "last contact date" in annotation.lower()
...
def generate_prompt_template(few_shot_examples, static_samples):
    return f"""Task: You are an expert medical annotator specializing in oncology pathology. Your task is to extract the last contact date from the provided medical note and output it in a precise, templated format.
