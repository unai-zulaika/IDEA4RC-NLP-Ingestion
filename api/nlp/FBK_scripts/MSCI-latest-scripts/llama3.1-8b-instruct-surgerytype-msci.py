...
def filter_annotations(example):
    ...
    return "surgery was performed" in annotation.lower()
...
def generate_prompt_template(few_shot_examples, static_samples):
    return f"""Task: You are a medical text annotation assistant. Given a clinical note, extract structured surgery-related information.
...
#        preds = preds.split('\n')[0]
    ...
    if 'macroscopically incomplete' in preds and 'macroscopically complete' in annt:
        preds = ""
    elif 'macroscopically complete' in annt and 'macroscopically incomplete' in preds:
