...
def filter_annotations(example):
    ...
    return example["annotation"].startswith("Patient\'s weight") or example["annotation"].startswith("Patient\'s BMI/weight")
...
def generate_prompt_template(few_shot_examples):
    return f"""You are a clinical information extraction assistant specialized in structured oncology metadata.
...
ANN_RE = re.compile(
    r"Patient's weight\s*(?P<weight>\d+(?:\.\d+)?)\s*kg\s*and\s*height\s*(?P<height>\d+(?:\.\d+)?)\s*cm\s*\(evaluated on\s*(?P<date>\d{2}\/\d{2}\/\d{4})\)",
    re.IGNORECASE
)
...
#    print(m)
    if not m:
        return {"weight": None, 'height': None, "date": None}
    weight   = m.group("weight").strip()
    height   = m.group("height").strip()
    date  = m.group("date").strip().lower().strip('.')
    return {"weight": weight, 'height': height, "date": date}
...
def slot_text(s):
    ...
    return "@".join([
        slots["weight"] or "unknown",   # normalize date placeholder
        slots["height"] or "unknown",   # normalize date placeholder
        slots["date"] or "unknown"              # normalize place placeholder
    ])
...
def compare_and_join(list1, list2):
    ...
    if len(list1) != 3 or len(list2) != 3:
        raise ValueError("Each list must contain exactly two tokens.")
    ...
    if total_matches == 3 or total_matches == 0:
        joined1 = " ".join(list1)
        joined2 = " ".join(list2)
        print('case Both matches', list1, ' | ', list2, ' | ', joined1, ' | ', joined2)
        flg = 0
        return joined1, joined2, flg

    if total_matches < 3 and total_matches > 0:
        joined1 = " ".join(list1)
        joined2 = " ".join(list2)
        print('case Both matches', list1, ' | ', list2, ' | ', joined1, ' | ', joined2)
        flg = 1
...
#    elif matches[0] and '[put value]' not in list1[0]:
#        joined1 = list1[0]
#        joined2 = list2[0]
#        print('\ncase 1st matches', list1, ' | ', list2, ' | ', joined1, ' | ', joined2)
#        flg = 1
#        return joined1, joined2, flg
#
#    elif matches[1] and '[put date]' not in list2[1]:
#        joined1 = list1[1]
#        joined2 = list2[1]
#        print('\ncase 2nd matches', list1, ' | ', list2, ' | ', joined1, ' | ', joined2)
#        flg = 1
#        return joined1, joined2, flg
...
def slot_cosine(gold, pred):
    gtxt1 = slot_text(gold)
    ptxt1 = slot_text(pred)
    print('\nBefore: ')
    print(gtxt1)
    print(ptxt1)
    gtxt, ptxt, flg = compare_and_join(gtxt1.split('@'), ptxt1.split('@'))
    print('\nAfter: ')
    print(gtxt)
    print(ptxt)
    vec = TfidfVectorizer()
    X = vec.fit_transform([gtxt, ptxt])
    return cosine_similarity(X)[0,1], flg
...
#    print('\n----\nGenerated (Raw) Annotation:\n', generated_text[0].split('### Response:\nAnnotation:')[1].split('\n')[1].strip())
    ...
    cossim, flg = slot_cosine(annt, preds)
    cossim = round(cossim, 2)
