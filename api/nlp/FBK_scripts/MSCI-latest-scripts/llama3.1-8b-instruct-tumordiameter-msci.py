...
def filter_annotations(example):
    ...
    return example["annotation"].startswith("Tumor longest diameter")
...
def generate_prompt_template(few_shot_examples):
    return f"""You are an expert medical annotator specializing in oncology pathology. Your task is to extract extract the tumor longest diameter (in mm) and the date from the provided medical note and output it in a precise, templated format.
...
#        preds = preds.split('\n')[0]
        if 'Tumor longest diameter: unknown' in preds or 'Tumor longest diameter: Unknown' in preds:     
            preds = 'Tumor longest diameter unknown.'
        preds = re.sub(r'\s+(?=(cm|mm)\b)', r'', preds, flags=re.IGNORECASE)    
