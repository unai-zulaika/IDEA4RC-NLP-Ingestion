...
def filter_annotations(example):
    ...
    return example["annotation"].startswith("Status of the patient at last follow-up")
# Apply filter
...
def generate_prompt_template(few_shot_examples, static_samples):
    return f"""Task: From the given medical note, extract the status of the patient at last follow-up.
