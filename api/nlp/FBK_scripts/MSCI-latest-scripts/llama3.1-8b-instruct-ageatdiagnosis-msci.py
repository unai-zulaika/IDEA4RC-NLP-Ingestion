...
def filter_annotations(example):
    ...
    return "age at diagnosis " in annotation.lower()
...
def generate_prompt_template(few_shot_examples, static_samples):
    return f"""You are an expert medical annotator specializing in extracting structured clinical information from oncology notes. Identify the age at diagnosis of the patient from the given medical note and output it in the exact format below.
...
#        preds = preds.split('\\n')[0]
        if preds.startswith('Age at diagnosis') != True:
            preds = 'Age at diagnosis ' + preds
...
#        preds = preds.split('\\n')[0]
    ...
    cossim = calculate_cosine_similarity(preds.split('\n')[0],annt)
