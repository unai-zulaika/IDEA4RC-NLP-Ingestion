...
mask = merged_df['annotation'].str.contains('No information about patient gender.', na=False)
merged_df.loc[mask, 'annotation'] = "Patient's gender unknown"

merged_df = merged_df.dropna(subset=['annotation', 'note_original_text'])
...
#    return "patient's gender" in annotation.lower()
    return "patient" in annotation.lower() and "gender" in annotation.lower()
...
def generate_prompt_template(few_shot_examples, static_samples):
    return f"""You are an expert medical annotator specializing in extracting structured clinical information from medical notes. 
