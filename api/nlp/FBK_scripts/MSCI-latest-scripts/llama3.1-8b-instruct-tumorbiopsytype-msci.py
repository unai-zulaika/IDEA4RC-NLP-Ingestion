...
df1 = pd.read_csv('clinical_entities/MSCI_data/MSCI_Data_25_patients.csv')
df1 = df1.dropna(subset=['annotation', 'note_original_text'])

df2 = pd.read_csv('clinical_entities/MSCI_data/MSCI_Data_25_patients_phase2_data.csv')
df2 = df2.dropna(subset=['annotation', 'note_original_text'])

# Print sizes of the original DataFrames
print(f"\n\nSize of Phase 1 Data: {df1.shape}")
print(f"Size of Phase 2 Data: {df2.shape}")

# Merge DataFrames
merged_df = pd.concat([df1, df2], ignore_index=True)
print(f"Size of Merged Dataset: {merged_df.shape}\n\n")
...
def filter_annotations(example):
    ...
    return example["annotation"].startswith("Baseline/primary tumor")
...
def generate_prompt_template(few_shot_examples):
    return f"""You are a clinical information extraction assistant.  
...
TEMPLATE_WORDS = set("baseline/primary tumor biopsy has been performed on in".split())

import re
from datetime import datetime

# --- Canonicalization of biopsy types (extend as needed) ---
BIOPSY_MAP = {
    # Swedish → canonical
    "finnålsbiopsi": "FNA",
    "fna": "FNA",
    "mellannålsbiopsi": "core-trucut",
    "borrbiopsi": "core-trucut",
    "tru-cut": "core-trucut",
    "trucut": "core-trucut",
    "core needle": "core-trucut",
    "excision": "excisional",
...
def canonical_biopsy(s: str) -> str:
    s = s.lower().strip()
    for k, v in BIOPSY_MAP.items():
        if k in s:
            return v
    # fall back to known labels if already in canonical form
    if s in {"fna", "core-trucut", "excisional", "incisional"}:
        return s
    return s  # as-is if truly unknown

# --- Parse the structured Annotation ---
ANN_RE = re.compile(
#    r"Baseline/primary tumor\s+(?P<biopsy>.+?)\s+biopsy has been performed on\s+(?P<date>.+?)\s*$",
    r"Baseline/primary tumor\s+(?P<biopsy>.+?)\s+biopsy has been performed on\s+(?P<date>.+?)\s+in\s+(?P<place>.+?)\.\s*$",
    re.IGNORECASE
)

#def parse_annotation(s: str):
#    m = ANN_RE.search(s.strip())
#    if not m:
#        return {"biopsy": None, "date": None}
#    biopsy = canonical_biopsy(m.group("biopsy"))
#    date   = m.group("date").strip()
#    return {"biopsy": biopsy, "date": date}

def parse_annotation(s: str):
    m = ANN_RE.search(s.strip())
#    print(m)
    if not m:
        return {"biopsy": None, "date": None, "place": None}
    biopsy = canonical_biopsy(m.group("biopsy"))
    date   = m.group("date").strip()
    place  = m.group("place").strip().lower()
    return {"biopsy": biopsy, "date": date, "place": place}

#def slot_text(s):
#    slots = parse_annotation(s)
#    return " ".join([
#        slots["biopsy"] or "",
#        (slots["date"] or "").replace("[put date]","")
#    ])

def slot_text(s):
    slots = parse_annotation(s)
    return "@".join([
        slots["biopsy"] or "unknown",
        slots["date"] or "unknown",   # normalize date placeholder
        slots["place"] or "unknown"              # normalize place placeholder
    ])

def sanitize(text):
    return " ".join(w for w in text.lower().split() if w not in TEMPLATE_WORDS)

def compare_and_join(list1, list2):

    # Ensure both lists have exactly 3 elements
    if len(list1) != 3 or len(list2) != 3:
        raise ValueError("Each list must contain exactly three tokens.")

    # Identify matching positions
    matches = [list1[i] == list2[i] for i in range(3)]
    total_matches = sum(matches)

    # Case 1: First token doesn't match → keep only the first token
    if not matches[0]:
        return list1[0], list2[0]

#    # Case 2: All three match or none match
#    if total_matches == 3 or total_matches == 0:
#        joined1 = " ".join(list1)
#        joined2 = " ".join(list2)
#        return joined1, joined2
#
#    # Case 3: First matches + either 2nd or 3rd matches
#    elif matches[0] and (matches[1] or matches[2]):
#        matched_tokens1 = [list1[i] for i in range(3) if matches[i]]
#        matched_tokens2 = [list2[i] for i in range(3) if matches[i]]
#        joined1 = " ".join(matched_tokens1)
#        joined2 = " ".join(matched_tokens2)
#        return joined1, joined2

    # Otherwise, default to joining all (no clear rule given for partials)
    else:
        return " ".join(list1), " ".join(list2)

def slot_cosine(gold, pred):
    gtxt = sanitize(slot_text(gold))
    ptxt = sanitize(slot_text(pred))
    print('Before: ')
    print(gtxt)
    print(ptxt)
    gtxt, ptxt = compare_and_join(gtxt.split('@'), ptxt.split('@'))
    print('After: ')
    print(gtxt)
    print(ptxt)
    vec = TfidfVectorizer()
    X = vec.fit_transform([gtxt, ptxt])
...
#        print('\n[BEFORE]---Generated (Raw) Annotation:\n', pred1)
        preds = generated_text[0].split('### Response:\n')[1].split('\n')[0].replace('Annotation:', '').strip().replace('<|im_end|>','')
#        preds = preds.split('\n')[0]
...
#    vec = TfidfVectorizer()
#    X = vec.fit_transform([sanitize(slot_text(annt)), sanitize(slot_text(preds))])
#    cossim = cosine_similarity(X)[0,1]
    cossim = slot_cosine(annt, preds)
    cossim = round(cossim, 2)
...
print('\n# Exact Matches: ', em)
print('# Partial Matches: ', cs)
print('Accuracy(only EM): ', ((em) / len(filtered_test)) * 100)
print('Accuracy (EM+PM): ', ((em + cs) / len(filtered_test)) * 100)
