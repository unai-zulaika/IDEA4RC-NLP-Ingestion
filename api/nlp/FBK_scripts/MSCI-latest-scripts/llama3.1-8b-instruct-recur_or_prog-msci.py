...
def filter_annotations(example):
    ...
    return "there was a progression" in annotation.lower() or "there was no progression/recurrence" in annotation.lower() or "there was a recurrence" in annotation.lower()
...
def generate_prompt_template(few_shot_examples, static_samples):
    return f"""Task: You are a medical text annotation assistant. Your task is to determine if the note indicates progression, recurrence, no progression, or no recurrence of the disease. If progression or recurrence is mentioned, also extract the date if available.
...
#        preds = preds.split('\n')[0]
    ...
    if 'There was a progression' in preds and 'There was a recurrence' in annt:
        preds = ""
    elif 'There was a progression' in annt and 'There was a recurrence' in preds:
#        annt = annt.replace('[put_date]', '').strip()       
       preds = ''
