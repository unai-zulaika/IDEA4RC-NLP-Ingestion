...
def preprocess_function_rag(example):
    few_shot_examples = get_rag_few_shot_examples(example, filtered_train, num_samples=30)
...
def slot_text(s):
    ...
    return " ".join([
        slots["biopsy"] or "",
        "date" if slots["date"] else "date",   # normalize date placeholder
        slots["place"] or "place"              # normalize place placeholder
    ])
