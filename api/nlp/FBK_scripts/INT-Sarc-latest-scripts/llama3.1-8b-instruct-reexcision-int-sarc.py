...
def filter_annotations(example):
    ...
    return "re-excision was performed" in annotation.lower()
...
def generate_prompt_template(few_shot_examples, static_samples):
    return f"""You are a clinical information extraction assistant specialized in oncological surgery documentation.
...
ANN_RE = re.compile(
    r"Re-excision was performed on\s+(?P<date>.+?)\s+and was macroscopically\s+(?P<result>.+?)\s*$",
    re.IGNORECASE
)
...
#    print(m)
    if not m:
        return {"date": None, "result": None}
    date   = m.group("date").strip()
    result  = m.group("result").strip().lower().strip('.')
...
def slot_text(s):
    ...
    return "@".join([
        ...
        slots["result"] or "unknown"              # normalize place placeholder
    ])
...
def compare_and_join(list1, list2):
    ...
    elif matches[0] and '[provide date]' not in list1[0]:
        joined1 = list1[0]
        joined2 = list2[0]
    ...
    elif matches[1] and '[select result]' not in list2[1]:
        joined1 = list1[1]
        joined2 = list2[1]
...
repetition_penalty=1.1  # Avoid repeated outputs
    ...
    print('\n\nTest Sample: ', tc)
    tc += 1
    annt = re.sub(r'\s+(?=(HPF|dpw|mm2)\b)', r'', annt, flags=re.IGNORECASE)    
...
#        print('\n[BEFORE]---Generated (Raw) Annotation:\n', pred1)
        ...
        preds = re.sub(r'\s+(?=(HPF|dpw|mm2)\b)', r'', preds, flags=re.IGNORECASE)   
        preds = preds.replace('Biopsy mitotic count: Unknown biopsy mitotic count', 'Unknown biopsy mitotic count') 
