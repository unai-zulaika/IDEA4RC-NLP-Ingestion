...
def filter_annotations(example):
    ...
    return example["annotation"].startswith("Histolog") or example["annotation"].startswith("Tipo istol")
...
print(df_filtered['annotation'][:50])
df_filtered['annotation'] = df_filtered['annotation'] \
    .str.replace('Tipo istologico', 'Histological Type', regex=False) \
    .str.replace('seleziona il codice', 'select', regex=False) \
    .str.replace('ICD-O-3])', 'ICD-O-3 code])', regex=False)
print(df_filtered['annotation'][:50])
